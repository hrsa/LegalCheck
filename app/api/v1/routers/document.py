...
async def upload_document(background_tasks: BackgroundTasks, company_id: int = Form(...), file: UploadFile = File(...), db: AsyncSession = Depends(get_async_session)):
    ...
    return await save_document(db, DocumentCreate(**document_data), background_tasks)
